...
void rotate_z_vector(double theta, Vector v, Vector *rv)
{
  double c, s;
...
void rotate_x_vector(double theta, Vector v, Vector *rv)
{
  double c, s;
...
int main(int argc, char** argv)
{
  ...
  for(step=0; step<step0+1; step++) {
    ...
    double a_neptune, e_neptune, i_neptune, longnode_neptune, argperi_neptune, mean_anom_neptune;
    Vector zhat, pole, vtmp, normal;
    State tmp, tmp2;
    ...
    angle1 = atan2(8.347725480676398e-3, 2.6262745512520353e-2) + PI/2.0;
    angle2 = acos(0.9996202197221923);


    rotate_z(-angle1, pj[NEPTUNE], &tmp);
    rotate_x(-angle2, tmp, &tmp2);
    keplerian(kc[NEPTUNE], tmp2, &a_neptune, &e_neptune, &i_neptune, &longnode_neptune, &argperi_neptune, &mean_anom_neptune);

    zhat.x = 0.; zhat.y = 0.; zhat.z = 1.;
    pole = quaternion_rotate_vector(rr.q, zhat);
    rotate_z_vector(-angle1, pole, &vtmp);
    rotate_x_vector(-angle2, vtmp, &pole);

    normal.x = sin(i_neptune)*sin(longnode_neptune);
    normal.y = -sin(i_neptune)*cos(longnode_neptune);
    normal.z = cos(i_neptune);

    /* angle = pv0(atan2(pole.y, pole.x) - atan2(normal.y, normal.x)); */
    angle = pv0(atan2(pole.x, -pole.y) - atan2(normal.x, -normal.y));

    printf("(%.10le %.10le %.10le %.10le %.10le %.10le %.10le %.10le %.10le)\n", 
	   time/365.25, normal.x, normal.y, normal.z, pole.x, pole.y, pole.z, angle, (180./PI)*acos(pole.z));
