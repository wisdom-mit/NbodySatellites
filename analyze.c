...
int main(int argc, char** argv)
{
  ...
  for(step=0; step<step0+1; step++) {
    ...
    double a_body, e_body, i_body, longnode_body, argperi_body, mean_anom_body;
    double ip_sat;
    Vector zhat;
    double kcsave;
    ...
    kcsave = -1.;
    body = link[lk].i;
    keplerian(kc[body], pj[body], &a_body, &e_body, &i_body, &longnode_body, &argperi_body, &mean_anom_body);
    kcsave = kc[body];
    ...
    if(lk < nsatellites) {
      a_body = a_body/Re;
      ip_sat = acos(pole.x*sin(i_body)*sin(longnode_body) 
		    - pole.y*sin(i_body)*cos(longnode_body)
		    + pole.z*cos(i_body));
    }

    printf("(%.10le %d %.10le %.10le %.10le %.10le %.10le %.10le %.10le %.10le %.10le %.10le %.10le)\n", 
	   time/365.25, lk, 
	   a_body, e_body, i_body, ip_sat, longnode_body, argperi_body, mean_anom_body, pole.x, pole.y, pole.z, kcsave);
